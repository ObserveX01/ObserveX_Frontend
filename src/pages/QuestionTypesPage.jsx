      ...
      <main className="flex-1 ml-64 p-12">
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-y-10 gap-x-12 max-w-5xl">
            ...
            <UnavailableType
              icon={<ListChecks size={24} className="text-pink-600" />}
              label="Multi-select"
            />

            {/* 3. True or false (UNAVAILABLE) */}
            <UnavailableType
              icon={<CircleCheck size={24} className="text-pink-600" />}
              label="True or false"
            />

            {/* 4. Fill in the blanks (UNAVAILABLE) */}
            <UnavailableType
              icon={<PencilLine size={24} className="text-pink-600" />}
              label="Fill in the blanks"
            />

            {/* 5. Open ended (UNAVAILABLE) */}
            <UnavailableType
              icon={<AlignLeft size={24} className="text-pink-600" />}
              label="Open ended"
            />
...
const UnavailableType = ({ icon, label }) => (
  <div className="group relative flex items-center gap-4 p-2 cursor-not-allowed overflow-visible">
    ...
    {/* CONTENT (GREYED OUT) */}
    <div className="bg-pink-50/50 p-3 rounded-lg opacity-40 group-hover:opacity-10 transition-opacity">
      {icon}
    </div>
