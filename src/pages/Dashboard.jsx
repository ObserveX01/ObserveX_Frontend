...
const Dashboard = () => {
  ...
  const handleTakeExam = (name) => {
    if (userRole === "Student") {
      ...
      setError(
        "ACCESS DENIED: You are logged in as a Teacher. Only Students can participate in exams.",
      );
        ...
        {error && (
          <div className="mb-6 flex items-center justify-between bg-red-50 border-l-8 border-red-600 p-5 rounded-xl shadow-lg animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="flex items-center gap-4">
              ...
              <p className="text-red-800 font-black text-sm uppercase tracking-wide">
                {error}
              </p>
            </div>
            <button
          ...
          {filteredCourses.map(([name, qs]) => (
            ...
            >
              <div className="h-40 bg-gradient-to-br from-[#1e293b] to-[#334155] p-6 flex flex-col justify-center items-center text-white relative">
                <Sparkles
                  size={40}
                  className="mb-2 opacity-20 absolute top-4 right-4 rotate-12"
                />
                <h3 className="text-lg font-black uppercase text-center leading-tight z-10">
                  {name}
                </h3>
              </div>
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                >
                  {userRole === "Teacher" ? "Exam Blocked" : "Take Exam"}{" "}
                  <ArrowRight size={14} />
                </button>
              </div>
            </div>
