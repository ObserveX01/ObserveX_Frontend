...
const ExamPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const rRes = await fetch(
          `http://localhost:5142/api/results/latest/${studentEmail}/${decodedCourse}`,
        );
        if (rRes.ok) {
  ...
  const saveResultToDatabase = useCallback(
    async (finalScore, total, answersList) => {
      ...
      try {
        ...
        if (response.ok) {
          console.log(
            "✅ Data successfully synced with ExamResults & StudentAnswers",
          );
        } else {
  ...
  useEffect(() => {
    const handleFatalViolation = async (reason) => {
      if (
        ...
        if (email) {
          try {
            await fetch("http://localhost:5142/api/violations/log", {
              method: "POST",
              headers: {
              ...
              body: JSON.stringify({
                studentEmail: email,
                courseName: decodeURIComponent(courseName),
                teacherEmail: questions[0]?.teacherEmail,
                message: reason,
              }),
            });
          } catch (err) {
    ...
    const onVisibilityChange = () => {
      if (document.visibilityState === "hidden")
        handleFatalViolation("TAB SWITCHING DETECTED");
    };
    ...
    if (webcamRef.current && isStarted && !isFinished && !isTerminated) {
      ...
      try {
        const response = await fetch(
          "http://localhost:5142/api/proctor/analyze",
          {
            method: "POST",
            body: formData,
          },
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-screen font-black text-slate-400">
        SECURE LOADING...
      </div>
    );
              ...
              >
                ...
                <div className="bg-[#e22d2d] text-white px-8 py-4 rounded-3xl mb-8 inline-block shadow-lg">
                  <p className="text-xl font-black uppercase tracking-wide">
                    {aiWarning || "PROTOCOL BREACH"}
                  </p>
                </div>

                {/* Subtext */}
                <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-sm mx-auto mb-10">
                  The ObserveX AI monitoring system has identified a protocol
                  breach. This incident has been logged.
                </p>
...
const RuleItem = ({ icon, title, desc }) => (
  <div className="flex gap-4 p-4 rounded-2xl bg-white border border-slate-100 shadow-sm">
    ...
    <div>
      <h4 className="font-black text-slate-800 uppercase text-sm mb-1 tracking-tight">
        {title}
      </h4>
